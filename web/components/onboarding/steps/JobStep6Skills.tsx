  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl sm:text-3xl ds-heading mb-3">Benötigte Skills 🎯</h2>
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl text-center">
        ...
        {requiredSkills.length >= 3 && requiredSkills.length <= 5 && (
          <p className="text-sm text-green-600 mt-1">Perfekte Anzahl für gutes Matching! 🎯</p>
        )}
        {requiredSkills.length > 5 && (
          <p className="text-sm text-orange-600 mt-1">Viele Skills können die Bewerberanzahl reduzieren ⚠️</p>
        )}
      </div>

      {/* Skills Grid */}
      <div>
        <label className="ds-label mb-3">Verfügbare Skills * (mind. 1)</label>
              ...
              <button
                ...
                className={`text-sm px-3 py-2.5 rounded-lg border-2 transition-all duration-300 font-medium ${
                  requiredSkills.includes(skill) 
                    ? 'ds-skill-tag-green border-green-500 scale-105' 
                    : 'ds-skill-tag-default hover:border-green-300'
                }`}
              >
                {requiredSkills.includes(skill) && "✓ "}
                {skill}
      ...
      {requiredSkills.length > 0 && (
        <div>
          <p className="text-sm ds-body-light mb-2 font-medium">Ausgewählte Skills:</p>
          <div className="flex flex-wrap gap-2">
            {requiredSkills.map(skill => (
              <span key={skill} className="inline-flex items-center gap-2 ds-skill-tag-green text-sm">
                {skill}
                <button 
                  onClick={() => toggleSkill(skill)}
                  className="hover:text-[var(--accent-green-dark)] transition-colors duration-300 font-bold"
                >
                  ×
                </button>
              </span>
      ...
      <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-green-800">
            <strong>Pro-Tipp:</strong> 3-5 Skills sind ideal! Zu viele Skills können qualifizierte Bewerber ausschließen.
          </p>
        </div>
      </div>
    </div>
