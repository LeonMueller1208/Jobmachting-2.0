...
interface Step6IndustryProps {
  industry: string;
  setIndustry: (value: string) => void;
  onSkip: () => void;
}

export default function Step6Industry({ industry, setIndustry, onSkip }: Step6IndustryProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl sm:text-3xl ds-heading mb-3">Deine Branche 🏢</h2>
        <p className="ds-body-light text-base sm:text-lg">In welcher Branche möchtest du arbeiten?</p>
      </div>

      {/* Industry Dropdown */}
      <div>
        <label className="ds-label">Bevorzugte Branche (optional)</label>
        <select
          value={industry}
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-blue-800">
            Diese Angabe ist optional. Du siehst Jobs aus allen Branchen, aber wir priorisieren deine Präferenz.
          </p>
        </div>
      </div>
    </div>
