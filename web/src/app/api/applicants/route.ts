...
export async function POST(request: Request) {
  try {
    ...
    const upserted = await prisma.applicant.upsert({
      where: { email },
      update: { name, skills, location, experience: Number(experience) || 0, bio: bio || null, industry: industry || null },
      create: { email, name, skills, location, experience: Number(experience) || 0, bio: bio || null, industry: industry || null },
    });
